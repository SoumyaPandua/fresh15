...
import {
  ...
  deleteMySupportTicketService,
  deleteAdminSupportTicketService,
...
export const addSupportMessage = async (req, res) => {
  try {
    return sendResponse(
      ...
      await createSupportMessageService(req.params.id, req.user._id, req.user.role, req.body.message),
    );
  } catch (error) { return sendError(res, error); }
};
