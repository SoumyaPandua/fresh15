...
export const getAdminDashboardService = async () => {
    ...
    const [
        ...
        Inventory.find({
            ...
            .populate("productId", "name images sku sellingPrice stock")
            .limit(10),
        Order.aggregate([
            {
                $match: {
    ...
    const topProductIds = topSellingProducts
        .map((item) => item._id)
        .filter(Boolean);
    ...
    const topProductDocs = await Product.find({
        ...
        .select("name sellingPrice stock images")
        .lean();
    ...
    const maxCategoryRevenue = categorySales.reduce(
    ...
    const categoryMix = categorySales.map((item) => ({
        ...
        value:
            maxCategoryRevenue > 0
                ? Number(
                      (
                          (Number(item.revenue || 0) / maxCategoryRevenue) *
                          100
                      ).toFixed(1)
                  )
                : 0,
    ...
    const hourlyOrdersMap = new Map(
        hourlyOrders.map((item) => [Number(item._id), Number(item.orders || 0)])
    );
